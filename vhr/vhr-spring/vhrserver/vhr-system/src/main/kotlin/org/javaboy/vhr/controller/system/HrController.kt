package org.javaboy.vhr.controller.systemimport
...
class HrController {
    ...
    @PutMapping("/")
    fun updateHr(@RequestBody hr: Hr?): RespBean {
        return if (hrService!!.updateHr(hr) == 1) {
            ok("更新成功!")
    ...
    @PutMapping("/role")
    fun updateHrRole(hrid: Int?, rids: Array<Int?>): RespBean {
        return if (hrService!!.updateHrRole(hrid, rids)) {
            ok("更新成功!")
        } else RespBean.error("更新失败!")
    }

    @DeleteMapping("/{id}")
    fun deleteHrById(@PathVariable id: Int?): RespBean {
        return if (hrService!!.deleteHrById(id) == 1) {
            ok("删除成功!")
