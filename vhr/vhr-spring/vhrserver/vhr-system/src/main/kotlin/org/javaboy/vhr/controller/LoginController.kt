package org.javaboy.vhr.controllerimport
...
class LoginController {
    ...
    fun verifyCode(request: HttpServletRequest, resp: HttpServletResponse) {
        val code = VerificationCode()
        val image = code.image
        val text = code.text
        val session = request.getSession(true)
        session.setAttribute(Constants.Companion.VERIFY_CODE, text)
        ColorUtil.red(request.localAddr)
        ColorUtil.red(request.session.id)
        LoginController.log.info("验证码：" + session.getAttribute(Constants.Companion.VERIFY_CODE))
        VerificationCode.Companion.output(image, resp.outputStream)
    }
}