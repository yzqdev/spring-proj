...
class PositionController {
    ...
    @PostMapping("/")
    fun addPosition(@RequestBody position: Position?): RespBean {
        return if (positionService!!.addPosition(position) == 1) {
            ok("添加成功!")
        } else RespBean.error("添加失败!")
    }

    @PutMapping("/")
    fun updatePositions(@RequestBody position: Position?): RespBean {
        return if (positionService!!.updatePositions(position) == 1) {
            ok("更新成功!")
        } else RespBean.error("更新失败!")
    }

    @DeleteMapping("/{id}")
    fun deletePositionById(@PathVariable id: Int?): RespBean {
        return if (positionService!!.deletePositionById(id) == 1) {
            ok("删除成功!")
        } else RespBean.error("删除失败!")
    }

    @DeleteMapping("/")
    fun deletePositionsByIds(ids: Array<Int?>): RespBean {
        return if (positionService!!.deletePositionsByIds(ids) == ids.size) {
            ok("删除成功!")
        } else RespBean.error("删除失败!")
    }
}