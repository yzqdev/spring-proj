package org.javaboy.vhr.controller.salaryimport
...
class SalaryController {
    @Autowired
    var salaryService: SalaryService? = null

    @get:GetMapping("/")
    val allSalaries: List<Salary?>?
        get() = salaryService.getAllSalaries()

    @PostMapping("/")
    fun addSalary(@RequestBody salary: Salary): RespBean {
        return if (salaryService!!.addSalary(salary) == 1) {
            ok("添加成功!")
        } else RespBean.error("添加失败!")
    }

    @DeleteMapping("/{id}")
    fun deleteSalaryById(@PathVariable id: Int?): RespBean {
        return if (salaryService!!.deleteSalaryById(id) == 1) {
            ok("删除成功！")
        } else RespBean.error("删除失败！")
    }

    @PutMapping("/")
    fun updateSalaryById(@RequestBody salary: Salary?): RespBean {
        return if (salaryService!!.updateSalaryById(salary) == 1) {
            ok("更新成功!")
