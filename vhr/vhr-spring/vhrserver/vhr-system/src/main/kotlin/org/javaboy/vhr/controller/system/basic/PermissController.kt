...
class PermissController {
    ...
    @get:GetMapping("/menus")
    val allMenus: List<Menu?>?
        get() = menuService.getAllMenus()

    @GetMapping("/mids/{rid}")
    fun getMidsByRid(@PathVariable rid: Int?): List<Int?>? {
        return menuService!!.getMidsByRid(rid)
    }

    @PutMapping("/")
    fun updateMenuRole(rid: Int?, mids: Array<Int?>?): RespBean {
        return if (menuService!!.updateMenuRole(rid, mids)) {
            ok("更新成功!")
        } else RespBean.error("更新失败!")
    }

    @PostMapping("/role")
    fun addRole(@RequestBody role: Role?): RespBean {
        return if (roleService!!.addRole(role) == 1) {
            ok("添加成功!")
        } else RespBean.error("添加失败!")
    }

    @DeleteMapping("/role/{rid}")
    fun deleteRoleById(@PathVariable rid: Int?): RespBean {
        return if (roleService!!.deleteRoleById(rid) == 1) {
            ok("删除成功!")
