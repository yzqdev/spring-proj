  ...
  >
    <a-spin :spinning="formLoading">
      <el-form :form="form">
        ...
        >
          <el-input placeholder="请输入标题" v-decorator="['title', {rules: [{required: true, message: '请输入标题！'}]}]" />
        </el-form-item>
        <el-form-item
          label="类型"
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
        >
          <el-radio-group v-decorator="['type',{rules: [{ required: true, message: '请选择类型！' }]}]" >
            <el-radio-button v-for="(item,index) in typeDictTypeDropDown" :key="index" :value="item.code">{{ item.value }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item
          :labelCol="labelCol"
        ...
        <el-form-item class="subForm-item">
          <el-button type="primary" class="subButton" @click="handleSubmit('1')">发布</el-button>
          <el-button type="danger" class="subButton" @click="handleSubmit('0')">存为草稿</el-button>
          <el-button class="subButton" @click="handleCancel">取消</el-button>
        </el-form-item>
      </el-form>
    </a-spin>
  </el-dialog>
