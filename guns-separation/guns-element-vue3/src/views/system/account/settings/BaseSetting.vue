  ...
  <div class="account-settings-info-view">
    <el-row :gutter="16">
      <el-col :md="24" :lg="16">
        ...
        <el-form layout="vertical" :form="form">
          <el-form-item
            label="昵称"
          >
            <el-input placeholder="给自己起个昵称吧" v-decorator="['nickName']"/>
          </el-form-item>
          <el-form-item
            label="生日"
          >
          ...
          >
            <el-radio-group v-decorator="['sex',{rules: [{ required: true, message: '请选择性别！' }]}]" >
              <el-radio v-for="(item,index) in sexData" :key="index" :value="item.code">{{ item.name }}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item
            label="手机"
          ...
          <el-form-item
            label="电话"
          >
            <el-input placeholder="请输入电话" v-decorator="['tel', {rules: [{required: true, message: '请输入电话！'}]}]"/>
          </el-form-item>
          <el-form-item
            label="电子邮件"
            :required="false"
          >
            <el-input placeholder="请输入电子邮件地址" v-decorator="['email', {type: 'email',message: '请输入正确的邮箱号',rules: [{required: true, message: '请输入正确的邮箱号！'}]}]"/>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="submitUserInfo">更新基本信息</el-button>
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    initUserInfo () {
      setTimeout(() => {
        ...
        this.option.img = import.meta.env.VITE_APP_API_BASE_URL + '/sysFileInfo/preview?id=' + this.userInfo.avatar
        this.getSexData()
      }, 100)
    },
    /**
    ...
    setavatar (url) {
      this.option.img = import.meta.env.VITE_APP_API_BASE_URL + '/sysFileInfo/preview?id=' + url
      store.dispatch('GetInfo').then(() => {})
    }
  }
}
