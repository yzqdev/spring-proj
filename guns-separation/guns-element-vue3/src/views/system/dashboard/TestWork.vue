<template>
  <div>
    ...
    <div class="multi-tab-test">
      <h4>多标签组件测试功能</h4>
      <el-button @click="handleCloseCurrentTab" style="margin-right: 16px;">关闭当前页</el-button>
      <el-button @click="handleOpenTab" style="margin-right: 16px;">打开 任务列表</el-button>
      <a-popconfirm :visible="visible" @confirm="confirm" @cancel="cancel" okText="确定" cancelText="取消">
        <template v-slot:title>
          <div>
            <el-form :form="form" layout="inline">
              <el-form-item label="自定义名称">
                <el-input v-decorator="['tabName', {rules: [{required: true, message: '请输入新的 Tab 名称'}]}]"/>
              </el-form-item>
            </el-form>
          </div>
        </template>
        <el-button @click="() => visible = !visible" style="margin-right: 16px;">修改当前 Tab 名称</el-button>
      </a-popconfirm>
      ...
      <a-popconfirm :visible="visible2" @confirm="confirm2" @cancel="() => visible2 = false" okText="确定" cancelText="取消">
        <template v-slot:title>
          <div>
            ...
            <el-form :form="form2" layout="inline">
              <el-form-item label="页面KEY">
                <el-input v-decorator="['tabKey', { initialValue: '/dashboard/workplace' }]" />
              </el-form-item>
              <el-form-item label="自定义名称">
                <el-input v-decorator="['tabName', {rules: [{required: true, message: '请输入新的 Tab 名称'}]}]"/>
              </el-form-item>
        ...
        <el-button @click="() => visible2 = !visible2">修改某一个 Tab 名称</el-button>
      </a-popconfirm>
    </div>
    <el-divider />
    <div class="page-loading-test">
      <h4>全局遮罩测试</h4>
      <el-button @click="handleOpenLoading" style="margin-right: 16px;">打开遮罩(5s 自动关闭)</el-button>
      <el-button @click="handleOpenLoadingCustomTip">打开遮罩(自定义提示语)</el-button>
    </div>
  </div>
</template>
