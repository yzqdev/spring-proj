    ...
    <el-card :bordered="false">
          ...
          <el-row :gutter="48">
            <el-col :md="8" :sm="24">
              <el-form-item label="存储位置">
                <el-select v-model="queryParam.fileLocation" placeholder="请选择存储位置" >
                  <el-option v-for="(item,index) in fileLocationDictTypeDropDown" :key="index" :value="item.code" >{{ item.value }}</el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :md="8" :sm="24">
              <el-form-item label="文件仓库">
                <el-input v-model="queryParam.fileBucket" placeholder="请输入文件仓库"/>
              </el-form-item>
            </el-col>
            <template v-if="advanced">
              <el-col :md="8" :sm="24">
                <el-form-item label="文件名称">
                  <el-input v-model="queryParam.fileOriginName" placeholder="请输入文件名称（上传时候的文件名）"/>
                </el-form-item>
              </el-col>
            </template>
      ...
      >
        ...
        <span slot="fileSuffix" slot-scope="text">
          <el-tag color="blue">{{ text }}</el-tag>
        </span>
        <span slot="action" slot-scope="text, record">
          <a v-if="hasPerm('sysFileInfo:download')" @click="sysFileInfoDownload(record)">下载</a>
          <el-divider type="vertical" v-if="hasPerm('sysFileInfo:download') & hasPerm('sysFileInfo:detail')"/>
          <a v-if="hasPerm('sysFileInfo:detail')" @click="$refs.detailForm.detail(record)">详情</a>
          <el-divider type="vertical" v-if="hasPerm('sysFileInfo:detail') & hasPerm('sysFileInfo:delete')"/>
          <a-popconfirm v-if="hasPerm('sysFileInfo:delete')" placement="topRight" title="确认删除？" @confirm="() => sysFileInfoDelete(record)">
            <a>删除</a>
          </a-popconfirm>
          <el-divider type="vertical" v-if="(hasPerm('sysFileInfo:preview') & record.fileSuffix === 'png' || record.fileSuffix === 'jpeg' || record.fileSuffix === 'jpg'|| record.fileSuffix === 'gif'|| record.fileSuffix === 'tif' || record.fileSuffix === 'bmp' ) & hasPerm('sysFileInfo:delete')"/>
