<template>
  <el-card :bordered="false">
        ...
        <el-row :gutter="48">
          <el-col :md="8" :sm="24">
            <el-form-item label="参数名称">
              <el-input v-model="queryParam.name" allow-clear placeholder="请输入参数名称"/>
            </el-form-item>
          </el-col>
          <el-col :md="8" :sm="24">
          ...
          <template v-if="advanced">
            <el-col :md="8" :sm="24">
              <el-form-item label="所属分类">
                <el-select v-model="queryParam.groupCode" placeholder="请选择所属分类" allow-clear>
                  <el-option v-for="(item,index) in groupCodeDictTypeDropDown" :key="index" :value="item.code" >{{ item.value }}</el-option>
                </el-select>
              </el-form-item>
            </el-col>
          </template>
    ...
    <div class="table-operator" v-if="hasPerm('sysConfig:add')" >
      <el-button type="primary" v-if="hasPerm('sysConfig:add')" icon="plus" @click="$refs.addForm.add()">新增配置</el-button>
    </div>
    <s-table
      ref="table"
      size="default"
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <el-divider type="vertical" v-if="hasPerm('sysConfig:edit') & hasPerm('sysConfig:delete')"/>
