  ...
  <el-card :bordered="false" >
    <a-spin :spinning="loading">
          ...
          <el-row :gutter="48">
            <el-col :md="8" :sm="24">
              <el-form-item label="应用名称">
                <el-input v-model="queryParam.name" allow-clear placeholder="请输入应用名称"/>
              </el-form-item>
            </el-col>
            <el-col :md="8" :sm="24">
      ...
      <div class="table-operator" v-if="hasPerm('sysApp:add')" >
        <el-button type="primary" v-if="hasPerm('sysApp:add')" icon="plus" @click="$refs.addForm.add()">新增应用</el-button>
      </div>
      <s-table
        ref="table"
        size="default"
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <el-divider type="vertical" v-if="hasPerm('sysApp:edit') & hasPerm('sysApp:delete')" />
          <a-popconfirm v-if="hasPerm('sysApp:delete')" placement="topRight" title="确认删除？" @confirm="() => sysAppDelete(record)">
            <a>删除</a>
          </a-popconfirm>

          <el-divider type="vertical" v-if="hasPerm('sysApp:setAsDefault') & hasPerm('sysApp:delete') & record.active == 'N' || hasPerm('sysApp:edit') & hasPerm('sysApp:setAsDefault') & record.active == 'N'" />
