...
import {   BlankLayout, PageView,   Iframe } from '@/layouts'
import { defineAsyncComponent, markRaw } from 'vue'
const BasicLayout =markRaw( () => import('../layouts/BasicLayout.vue')  )
const RouteView =markRaw( () => import('../layouts/RouteView.vue')  )
// 前端路由表
...
const notFoundRouter = {
  path: '/:pathMatch(.*)', redirect: '/welcome', hidden: true
}
// 个人中心页面
const userAccount = [
  // account
...
export const generatorDynamicRouter = (data) => {
  return new Promise((resolve, reject) => {
    ...
    console.log(routers)
console.log(`%c这是routers`,`color:red;font-size:16px;background:transparent`)
    resolve(routers)
  }).catch(err => {
    // reject('加载菜单失败')
