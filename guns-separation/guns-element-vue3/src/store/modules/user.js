  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        getLoginUser().then(response => {
          if (response.success) {
            ...
            if (data.avatar != null) {
              commit('SET_AVATAR', import.meta.env.VITE_APP_API_BASE_URL + '/sysFileInfo/preview?id=' + data.avatar)
            }
            resolve(data)
          } else {
    ...
    Logout ({ commit, state }) {
      return new Promise((resolve) => {
        console.log(`%c退出登录`,`color:red;font-size:16px;background:transparent`)
      ...
      return new Promise((resolve, reject) => {
        sysDictTypeTree().then((data) => {
          if (data.success) {
            ...
            ls.set(DICT_TYPE_TREE_DATA, result)
            resolve()
          } else {
            // eslint-disable-next-line no-undef
            reject(new Error(data.message))
    ...
    MenuChange ({ commit }, application) {
      return new Promise((resolve) => {
        sysMenuChange({ application: application.code }).then((res) => {
          ...
          // 此时缓存中全部都是不默认的应用
          ls.set(ALL_APPS_MENU, new_false_all_app_menu)
          apps.name = application.name
          apps.code = application.code
          const applocationR = []
          applocationR.push(apps)
          ls.set(ALL_APPS_MENU, applocationR)
          resolve(res)
