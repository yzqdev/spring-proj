...
public class ConstantContextHolder   {
    ...
    public static Long getSessionTokenExpireSec() {
        return getSysConfigWithDefault("GUNS_SESSION_EXPIRE", Long.class,   60L);
    }
    ...
    public static Long getTokenExpireSec() {
        return getSysConfigWithDefault("GUNS_TOKEN_EXPIRE", Long.class, 30*86400L);
    }
    ...
    public static <T> T getSysConfig(String configCode, Class<T> clazz, Boolean nullThrowExp) {
        log.info(ConstantContext.me().toString());

        String configValue = ConstantContext.me().getStr(configCode);
        if (ObjectUtil.isEmpty(configValue)) {
            if (nullThrowExp) {
                String format = StrUtil.format(">>> getSysConfig 系统配置sys_config表中存在空项，configCode为：{}", configCode);
                log.error(format);
