  ...
  <Layout style="margin-top: 50px">
      ...
      <p style="position: fixed; right: 58px; z-index: 1; bottom: 68px">
        ...
        >
          ...
          <DropdownMenu slot="list">
            <DropdownItem @click.native="disableSelectUser"
              ><Icon type="md-close-circle" /> 禁用选中</DropdownItem
            >
            <DropdownItem @click.native="deleSelectUser"
    ...
    <Modal v-model="updateUserInfoMSG" :footer-hide="true">
      ...
      <Form @submit.native.prevent :label-width="80">
        ...
        <p style="text-align: center; color: #f56838">
          注意：欲要给用户分配分发群组，只能选择未指定用户组的分发群组
        </p>
        <br />
        <Button
          type="primary"
...
export default {
  ...
  methods: {
    getStorage() {
      request("/admin/root/getGrouplistForUsers", {})
        .then((res) => {
          if (res.status == 200) {
    ...
    getUserList() {
      ...
      request("/admin/root/getUserList", paramJson)
        .then((res) => {
    ...
    sendDeleUser(id) {
      ...
      request("/admin/root/deleUser", paramJson)
        .then((res) => {
          if (res.status == 200) {
    ...
    updateUserInfo() {
      ...
      if (
        this.formItem.id == "" ||
        this.formItem.email == "" ||
        this.formItem.memory == "" ||
        this.formItem.groupid == "" ||
        this.formItem.isOkSwitch == ""
      ) {
      ...
      request("/admin/root/updateUserInfo", paramJson)
        .then((res) => {
          if (res.status == 200) {
            if (res.data.code == "200") {
              this.$Message.success(res.data.info);
