<template>
  <Layout style="margin-top: 50px">
    <Content :style="{ margin: '15px 20px 0' }">
      <p style="position: fixed; right: 58px; z-index: 1; bottom: 68px">
        ...
        >
          ...
          <DropdownMenu slot="list">
            <DropdownItem @click.native="deleSelectCodes"
              ><Icon type="md-trash" /> 删除选中</DropdownItem
            >
            <DropdownItem @click.native="showAdd"
              ><Icon type="md-add-circle" />添 加</DropdownItem
            >
          </DropdownMenu>
        </Dropdown>
      </p>
      <Card
        class="cardShadow"
        style="margin-top: 10px; padding-left: 8px; padding-right: 8px"
      >
        <Table
          :loading="loading"
          ref="selection"
          :columns="columns"
          :data="codeList"
          @on-selection-change="selectCode"
        >
          ...
          <template slot-scope="{ row, index }" slot="action">
            <Icon
              type="ios-copy tablebut cobyOrderSn"
              :data="index"
              title="复制"
      ...
      <Form @submit.native.prevent :label-width="80">
        <FormItem label="生成数量">
          <Input
            ...
            type="number"
            placeholder="生成的扩容码数量"
            size="large"
          ></Input>
        </FormItem>
        <FormItem label="单位容量">
          <Input
            v-model="formItem.memory"
            type="number"
            placeholder="每个扩容码的容量,最大1048576"
            size="large"
          >
...
export default {
  ...
  methods: {
    ...
    getCodeList() {
      ...
      this.codeList = [];
      request("/admin/root/selectCodeList", paramJson)
        .then((res) => {
          if (res.status == 200) {
    ...
    addCodeList() {
      if (
        Number(this.formItem.memory) <= 0 ||
        Number(this.formItem.memory) > 1048576
      ) {
        this.$Message.warning("请输入正确的容量值:1~1048576");
        return;
      }
      if (
        Number(this.formItem.count) <= 0 ||
        Number(this.formItem.count) > 1000
      ) {
        this.$Message.warning("请输入正确的生成数量:1~1000");
        return;
      }
      this.addLoading = true;
      request("/admin/root/addCode", this.formItem)
        .then((res) => {
    ...
    sendDeleCodes(e) {
      ...
      request("/admin/root/deleteCodes", paramJson)
        .then((res) => {
          if (res.status == 200) {
            if (res.data.code == "200") {
              this.$Message.success(res.data.info);
