  ...
  <Layout style="margin-top: 50px">
        ...
        >
          ...
          <template slot-scope="{ row, index }" slot="action">
            <Icon
              ...
              @click="showsettingGroup(row, index)"
            />
    ...
    <Modal v-model="isAdd" :footer-hide="true">
      ...
      <Form @submit.native.prevent :label-width="70">
        <FormItem label="群组名称">
          <Input
            :disabled="formItem.id == 1 && urlType == 'edit'"
        ...
        <div style="color: rgb(228 102 70); font-size: 12px; font-weight: 200">
          <p>分发群组优先级判定：</p>
          <p>
            未指定用户组的分发群组且已经分配给指定用户使用的优先级最高，其次是设置有用户组的分发群组。若以上所属的分发群组都没有指定，则一律进默认群组。
          </p>
          <br />
        </div>
...
export default {
  ...
  methods: {
    ...
    getGroupList() {
      ...
      request("/admin/root/getGroupList", paramJson)
        .then((res) => {
          if (res.status == 200) {
    ...
    save() {
      ...
      request("/admin/root/" + this.urls, this.formItem)
        .then((res) => {
          if (res.status == 200) {
    ...
    sendDeleGroup(id) {
      ...
      request("/admin/root/deleGroup", paramJson)
        .then((res) => {
          if (res.status == 200) {
            if (res.data.code == "200") {
              this.$Message.success(res.data.info);
