    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          console.log(`%ccrud获取数据`,`color:red;font-size:16px;background:transparent`)
          console.log(crud)
          // time 毫秒后显示表格
          setTimeout(() => {
            crud.loading = false
...
function callVmHook(crud, hook) {
  ...
  console.log(`%ccallVmHook`,`color:red;font-size:16px;background:transparent`)
  console.log(crud)
  console.log(hook)
 ...
 */
function crud(options = {}) {
  console.log(`%c先走crud`,`color:red;font-size:16px;background:transparent`)
  const defaultOptions = {
    type: undefined
  }
  options = mergeOptions(defaultOptions, options)
