...
io.swagger.v3.oas.annotations.tags.Tagimport lombok.RequiredArgsConstructorimport me.zhengjie.annotation .Logimport me.zhengjie.base.BaseEntityimport me.zhengjie.exception.BadRequestExceptionimport me.zhengjie.modules.system.domain.Dictimport me.zhengjie.modules.system.rest.DictControllerimport me.zhengjie.modules.system.service.DictServiceimport me.zhengjie.modules.system.service.dto.DictQueryCriteriaimport org.springframework.data .domain.Pageableimport org.springframework.http.HttpStatusimport org.springframework.http.ResponseEntityimport org.springframework.security.access.prepost.PreAuthorizeimport org.springframework.validation.annotation .Validatedimport org.springframework.web.bind.annotation .*import java.io.IOExceptionimport
...
class DictController {
    ...
    fun download(response: HttpServletResponse?, criteria: DictQueryCriteria?) {
    ...
    fun queryAll(): ResponseEntity<Any> {
        return ResponseEntity(dictService!!.queryAll(DictQueryCriteria()), HttpStatus.OK)
    }

    @Operation(summary = "查询字典")
    @GetMapping
    @PreAuthorize("@el.check('dict:list')")
    fun query(resources: DictQueryCriteria?, pageable: Pageable?): ResponseEntity<Any> {
        return ResponseEntity(dictService!!.queryAll(resources, pageable), HttpStatus.OK)
    }
    ...
    fun create(@Validated @RequestBody resources: Dict): ResponseEntity<Any> {
        if (resources.id != null) {
            throw BadRequestException("A new " + DictController.Companion.ENTITY_NAME + " cannot already have an ID")
        }
    ...
    companion object {
        private const val ENTITY_NAME = "dict"
    }
}