...
class MinIOServiceImpl : AbstractS3BaseFileService(), BaseFileService {
    override fun init(driveId: String?) {
        ...
        val region = stringStorageConfigMap[StorageConfigConstant.REGION_KEY]!!.value
    ...
    override fun storageStrategyConfigList(): List<StorageConfig> {
        return object : ArrayList<StorageConfig?>() {
            init {
                ...
                add(StorageConfig("endPoint", "服务地址"))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(MinIOServiceImpl::class.java)
    }
}