...
class TencentServiceImpl : AbstractS3BaseFileService(), BaseFileService {
    override fun init(driveId: String?) {
        ...
        val secretId = stringStorageConfigMap[StorageConfigConstant.SECRET_ID_KEY]!!.value
        ...
        if (Objects.isNull(secretId) || Objects.isNull(secretKey) || Objects.isNull(endPoint) || Objects.isNull(
                bucketName
            )
        ) {
            ...
            val credentials = BasicAWSCredentials(secretId, secretKey)
            s3Client = AmazonS3ClientBuilder.standard()
                .withCredentials(AWSStaticCredentialsProvider(credentials))
                .withEndpointConfiguration(AwsClientBuilder.EndpointConfiguration(endPoint, "cos")).build()
            testConnection()
            isInitialized = true
        }
    }
    ...
    companion object {
        private val log = LoggerFactory.getLogger(TencentServiceImpl::class.java)
    }
}