...
class OneDriveChinaServiceImpl : AbstractOneDriveServiceBase(), BaseFileService {
    @Resource
    ...
    override fun init(driveId: String?) {
        ...
        isInitialized =
            if (StringUtils.isEmpty(accessToken) || StringUtils.isEmpty(
                ...
                OneDriveChinaServiceImpl.log.debug("初始化存储策略 [{}] 失败: 参数不完整", storageTypeEnum.description)
                false
            } else {
